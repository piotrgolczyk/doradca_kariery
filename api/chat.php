...
withLockedJsonFile($userPath, function (array $userState) use ($userMessage, $promptMain, $summarizerSystem, $apiKey, $flatTopics, $topicsData): array {
    ...
    if ($pending) {
        ...
        if (in_array($answer, ['tak', 'yes', 'y', 'potwierdzam'], true)) {
            ...
            return ['data' => $userState, 'return' => null, 'stream_text' => $confirmMessage];
        }
    ...
    foreach (($control['events'] ?? []) as $event) {
        if (($event['type'] ?? '') === 'topic_achieved') {
            ...
            if ($factKey && is_string($factValue) && trim($factValue) !== '') {
                upsertFact($userState['profile_facts'], $factKey, trim($factValue), $topicId, (float)($sum['confidence'] ?? 0.8));
            }
        }
